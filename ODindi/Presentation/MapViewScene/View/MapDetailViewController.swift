...
// MapViewController 에서 Annotation 탭 시 나타나는 ViewController
class MapDetailViewController: UIViewController {
    ...
    // 상영일, 상영정보 컬렉션뷰 관련 코드는 MainViewController 와 대부분 유사함
    func bind() {
        viewModel.selectedCinema
            .bind { [weak self] selectedCinema in
                self?.logoImageView.image = UIImage(named: selectedCinema.name)
                self?.nameLabel.text = selectedCinema.name
                self?.addressLabel.text = selectedCinema.address
            }
    ...
    // insta, navermap 버튼 탭 시 나타나는 webView 를 만드는 메서드
    // 버튼을 번갈아 가면서 탭 시 이전 버튼의 정보가 webView 에 남아있어, 매번 새로운 webView 인스턴스를 만들어줌
